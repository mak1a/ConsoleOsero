...
bnsGame::Sikou::Sikou() : m_bestPoint(12, utl::Point()), m_depthMax(1), m_isExperiment(false) {
...
bnsGame::Sikou::Sikou(const bool isSelected) : m_bestPoint(12, utl::Point()), m_depthMax(1), m_isExperiment(false) {
    for (int32_t y{}; y < 10; ++y) {
        for (int32_t x{}; x < 10; ++x) {
            m_evaluationValues[y][x] = 0;
        }
...
: m_bestPoint(12, utl::Point()), m_depthMax((isExperiment_) ? 1 : 5), m_evaluationValues(values_), m_isExperiment(isExperiment_) {
    if (!isExperiment_) {
        Mutation();
    }
}
...
int32_t bnsGame::Sikou::AlphaBeta(const int32_t isSelfOrEnemy_, Aspect& aspect_, int32_t alpha_, int32_t beta_, const int32_t depth_) {
    if (aspect_.MakeLegalPuts(isSelfOrEnemy_) <= 0) {
        if (aspect_.MakeLegalPuts((isSelfOrEnemy_ == Self) ? Enemy : Self) <= 0) {
            return aspect_.value;
        }

        return AlphaBeta((isSelfOrEnemy_ == Self) ? Enemy : Self, aspect_, alpha_, beta_, depth_ + 1);
    }
    ...
    for (const auto& stonePos : valids) {
        ...
        int32_t value{AlphaBeta((isSelfOrEnemy_ == Self) ? Enemy : Self, a, alpha_, beta_, depth_ + 1)};
...
bnsGame::utl::Point bnsGame::Sikou::Think(const int32_t isSelfOrEnemy_, Aspect aspect_) noexcept {
    ...
    int32_t value = AlphaBeta(isSelfOrEnemy_, aspect_, -999999, 999999, 0);
